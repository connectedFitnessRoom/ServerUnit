...
object JsonConvertor {
  ...
  implicit class AirJsonConverter(airQuality: Air) {
    def toJson: String = {
      s"""{
         |  "env_data": {
  ...
  implicit class NumberOfSessionsJsonConverter(period: String) {
    def toJson(year: Int, month: Option[Int], week: Option[Int], count: Option[Int] = None, counts: Seq[Int] = Seq()): String = {
  ...
  implicit class MeanExerciseTimeJsonConverter(period: String) {
    def toJson(year: Int, month: Option[Int], week: Option[Int], mean: Double, means: Seq[Double]): String = {
      val periodJson = period match {
        case "year" =>
